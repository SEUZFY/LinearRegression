...
def get_long_lat(file_nm):
    ...
    for r in records:
        r = r.split(',')
        x_set.append(round(float(r[4])*0.01,6)) # longtitude to x_set
        y_set.append(round(float(r[2])*0.01,6)) # latitude to y_set
    return x_set, y_set
...
def main():
    x_set, y_set = get_long_lat(r"D:\AlbertQ2\GEO1003\assignment2\SmartPhoneGPGGA_filtered.txt")
    ...
    #plt.scatter(x_set[200],y_set[200],marker = '*')
    plt.show()
